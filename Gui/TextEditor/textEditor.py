...
class TextEditor:  # смешать с классом Frame, имеющим меню/панель инструментов
    ...
    def onDoChange(self, findtext, changeto):
        ...
        if self.text.tag_ranges(SEL):  # сначала найти
            ...
            self.onFind(findtext)  # переход к слудующему
            self.text.update()  # принудительно обновление
    ...
    def onGrep(self):
        ...
        кодированного текста с испытуемой строкой;
        ...
        # немодальный диалог: ввод имени каталога, шаблон имени файла, искомая строка
        popup = Toplevel()
