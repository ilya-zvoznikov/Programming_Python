...
chunksize = int(
    1.4 * kilobytes)  # в оригинале - мегабайты; я поставил килобайты, чтобы резать маленькие текстовые файлы
...
if __name__ == '__main__':
    if len(sys.argv) == 2 and sys.argv[1] == '--help':
        print('Use: python split.py [file-to-split target-dir [chunksize in bytes]]')
    else:
        if len(sys.argv) < 3:
            interactive = True
        ...
        print('Splitting', absfrom, 'to', absto, 'by', chunksize, 'bytes')
